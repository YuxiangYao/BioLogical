...
public:
    ...
    DNS_Basic& SystemEvolution_Asy1(int steps, short *ss0, std::vector<bool> *flag, std::mt19937 &InRNG); // Allow invalid update
    DNS_Basic& SystemEvolution_Asy2(int steps, short *ss0, std::vector<bool> *flag, std::mt19937 &InRNG); // Must select one update
...
class DNS_JustSim: public DNS_Basic {
    public:
    DNS_JustSim();
    DNS_JustSim(int size, int L, int Steps);
    ~DNS_JustSim();
    DNS_JustSim& DynamicParaConfig(double *s_bias);// Set dynamic parameter, can be used repeatedly.
    DNS_JustSim& SelfEvolution(int Steps, int UpdateType);
    //double FinalDistance();
    Rcpp::IntegerVector Out2Robj(int Steps);
};

class DNS_OnceSim: public DNS_Basic {
    public:
    DNS_OnceSim();
    DNS_OnceSim(int size, int L);
    ~DNS_OnceSim();
    DNS_OnceSim& DynamicParaConfig(double *s_bias);// Set dynamic parameter, can be used repeatedly.
    DNS_OnceSim& SelfEvolution(int Steps, int UpdateType);
    DNS_OnceSim& FinalState(int *FinalSlot);
};
