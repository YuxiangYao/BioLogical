#' @title Show the percolation cases
#' @description This function visualizes the state distribution at each lattice 
#' point on square, triangular, or hexagonal lattices. Nodes are typically classified
#' into three categories: stable at "1", stable at "0", and oscillatory. The 
#' output of the function depends on the \code{ggplot} class.
#' @param Values an IntegerVector, the results from \link{DNS_Percolation}
#' @param Length an integer, the lngth of lattice
#' @param Height an integer, the height of lattice
#' @param Type an integer, 4=square; 3=triangle; 6=hexagon
#' @param ColorFills a CharacterVector, [1] unstable, [2]0-stable, [3]1-stable; 
#' The string should conform to color syntax, such as "\code{red}" or "\code{#FF0000}".
#' @param LineColor a string that describes the color of polygonal boundary. 
#' \code{NA} means the boundary is not displayed.
#' @param Linewidth a numeric value, the line width of polygonal boundary.
#' @return a \code{ggplot} object, show the percolation case.
#' @export 
#' 
