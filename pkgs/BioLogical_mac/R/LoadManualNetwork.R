#' @title Load a user-defined network
#' @description Instead of importing from files, this function enables users to 
#' define regulatory relationships and network structures to explore various 
#' system features.
#' @param Object A formatted data to record regulatory patterns; support \code{matrix},
#' \code{data.frame}, \code{igraph} object (if available).
#' @param WhichCols An IntegerVector, specifies which columns should serve as 
#' the source and target columns (Default: \code{c(1,2)}).
#' @param BoolFunList A list in which each element is a Boolean function, with 
#' the list length equal to the system size. If \code{NULL}, the function will 
#' generate a list of Boolean functions randomly based on the parameters 
#' \code{OBF_ratio} and \code{OBF_type}. (Default: \code{NULL})
#' @param OBF_ratio A numeric value in \code{[0,1]} representing the proportion 
#' of ordered Boolean function; this value is invalid if \code{BoolFunList} is 
#' \code{FALSE}.
#' @param OBF_type A character denotes the type of ordered Boolean function, 
#' acceptable parameters are detailed in \link{BoolFun_Generator}.
#' @return A four-element formatted list that records information of a Boolean 
#' network (See \link{BoolGRN_CellCollective}).
#' @export
#'
LoadManualNetwork<-function(Object, WhichCols=c(1,2), BoolFunList=NULL,
  ...
  # else {
  #   ran.seed=RandSeed;} # Note using the <R>'s random number.
