...
DNS_Basic& DNS_Basic::SystemEvolution_Asy1(int steps, short *ss0, std::vector<bool> *flag, std::mt19937 &InRNG){
    int ii, jj, kk, index;
    short tmp;
    std::uniform_real_distribution<double> runif(0,1);
    for(ii=0; ii<steps; ++ii){
        jj=(int)(runif(InRNG)*N);// Random select one.
        index=0;
        for(kk=0; kk<((int)(*inedge)[jj].size()); ++kk){
...
DNS_Basic& DNS_Basic::SystemEvolution_Asy2(int steps, short *ss0, std::vector<bool> *flag, std::mt19937 &InRNG){
    ...
    for(ii=0; (ii<steps)&&(candidate.size()>0); ++ii){
        ...
        jj=(int)(runif(InRNG)*candidate.size());
...
DNS_Derrida& DNS_Derrida::DerridaDamageSpread(int Steps, int UpdateType){
    ...
    switch(UpdateType){
        ...
        case 2: // Asynchronous random one
            DNS_Basic::SystemEvolution_Asy1(Steps,sss,pb, InnerRNG);
            DNS_Basic::SystemEvolution_Asy1(Steps,sss+N+N,pb, InnerRNG);
            break;
        case 3: // Asynchronous must valid one
            DNS_Basic::SystemEvolution_Asy2(Steps,sss,pb, InnerRNG);
            DNS_Basic::SystemEvolution_Asy2(Steps,sss+N+N,pb, InnerRNG);
            break;
        default:;
    }
    return *this;
...
DNS_Percolation& DNS_Percolation::PercolationModel(int Steps, int Windows, int UpdateType){
    ...
    switch(UpdateType){
        ...
        case 2: // Asynchronous random one
            DNS_Basic::SystemEvolution_Asy1(Steps,sss,pb, InnerRNG);
            DNS_Basic::SystemEvolution_Asy1(Windows,sss,flag, InnerRNG);
            break;
        case 3: // Asynchronous must valid one
            DNS_Basic::SystemEvolution_Asy2(Steps,sss,pb, InnerRNG);
            DNS_Basic::SystemEvolution_Asy2(Windows,sss,flag, InnerRNG);
            break;
        default:;
    }
    return *this;
...
// only simulation and output template states. [V]
DNS_JustSim::DNS_JustSim(): DNS_Basic(){
}
DNS_JustSim::DNS_JustSim(int size, int L, int Steps): DNS_Basic(size, L){
    sss=(short *)malloc(Steps*size*sizeof(short)); // Store temporary state
}
DNS_JustSim::~DNS_JustSim(){
    free(sss);
    sss=nullptr;
}
// Configure some dynamic parameters//[v]
DNS_JustSim& DNS_JustSim::DynamicParaConfig(double *s_bias){
    DNS_Aux_Bias(sss, N, s_bias, L);       // Set original system states
    return *this;
}
// Execute once simulation of damage-spread//[v]
DNS_JustSim& DNS_JustSim::SelfEvolution(int Steps, int UpdateType){
    std::vector<bool> tmp(N,false);// Useless here
    std::vector<bool> *pb=&tmp;
    std::mt19937 InnerRNG( (unsigned int)((1<<26)*unif_rand()) );// Generate a seed from R.
    short *spt=sss;
    switch(UpdateType){
        case 1: // Synchronous
            for(int ii=0; ii<(Steps-1); ++ii){
                DNS_Basic::SystemEvolution_Syn(1,spt,pb);
                spt+=N;
            }
            break;
        case 2: // Asynchronous random one
            for(int ii=0; ii<(Steps-1); ++ii){
                memcpy(spt+N,spt,sizeof(short)*N);
                DNS_Basic::SystemEvolution_Asy1(1,spt+N,pb,InnerRNG);
                spt+=N;
            }
            break;
        case 3: // Asynchronous must valid one
            for(int ii=0; ii<(Steps-1); ++ii){
                memcpy(spt+N,spt,sizeof(short)*N);
                DNS_Basic::SystemEvolution_Asy2(1,spt+N,pb,InnerRNG);
                spt+=N;
            }
            break;
        default:;
...
// Output information to R-format
Rcpp::IntegerVector DNS_JustSim::Out2Robj(int Steps){
    return Rcpp::IntegerVector(sss, sss + Steps*N);
}   

// Only simulation and ouput final states (Space-saving). []
DNS_OnceSim::DNS_OnceSim(): DNS_Basic(){
}
DNS_OnceSim::DNS_OnceSim(int size, int L): DNS_Basic(size, L){
    sss=(short *)malloc(2*size*sizeof(short)); // Store temporary state
}// Double for old/new exchange.
DNS_OnceSim::~DNS_OnceSim(){
    free(sss);
    sss=nullptr;
}
// Configure some dynamic parameters//[v]
DNS_OnceSim& DNS_OnceSim::DynamicParaConfig(double *s_bias){
    DNS_Aux_Bias(sss, N, s_bias, L);       // Set original system states
    return *this;
}
// Execute once simulation of damage-spread//[v]
DNS_OnceSim& DNS_OnceSim::SelfEvolution(int Steps, int UpdateType){
    std::vector<bool> tmp(N,false);// Useless here
    std::vector<bool> *pb=&tmp;
    std::mt19937 InnerRNG( (int)(INT32_MAX*unif_rand()) );// Generate a seed from R.
    switch(UpdateType){
...
DNS_OnceSim& DNS_OnceSim::FinalState(int *FinalSlot){
    for(int ii=0; ii<N; ++ii){
        FinalSlot[ii]=sss[ii];}
    return *this;
}
