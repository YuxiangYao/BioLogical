...
NetGraphFrame& NetGraphFrame::IsolatedPointedNode_D(int ID){
    ...
    int loc;// [NOTE] Some self-loop edges can be
    // Remove the parents & changed the degrees
    tmp=Network+ID;
    while(tmp->prev!=nullptr){
        loc=tmp->prev->code;
        static_cast<void>(DeleteChilds(&Network[loc],ID));
        OtDeg[loc]--;
        tmp=tmp->prev;}
...
NetGraphFrame& NetGraphFrame::BreakDownPointedEdge_D(int source, int target){
    static_cast<void>(DeleteChilds(&Network[source],target));
    static_cast<void>(DeleteParents(&Network[target],source));
    return *this;
}
...
// Delete one network (Free all memory).
// NetGraphFrame& NetGraphFrame::Delete_Network(){
//     NetGraphFrame::Reset_Network();// First reset.
//     free(InDeg);InDeg=OtDeg=Coding=nullptr;
//     free(Network);Network=nullptr;
//     free(Address);Address=nullptr;
//     NetPara.clear();
//     return *this;
// }
// Show one network.
...
// Return the feedback loop of network by each SCC.
NetGraphFrame& NetGraphFrame::FeedBackLoop(std::vector<int> &aSCC, std::vector<std::vector<int>> &FBLs){
    if(aSCC.size()<2){// Single node.
        std::vector<int> tmpvec(2);
        tmpvec[0]=aSCC[0];tmpvec[1]=aSCC[0];
        FBLs.push_back(tmpvec);}
    else {// Exist non-trivial loops
        std::unordered_set<int> compSet(aSCC.begin(), aSCC.end());
        std::unordered_map<int,std::vector<int>> adjList;// Adjacent list;
        // Set the adjList
        for(int ii:aSCC){
            std::vector<int> Childs;
            struct Regulation *npt=&(Network[ii]);
            while(nullptr!=npt->next){
                Childs.push_back(npt->next->code);
        ...
        // Johnson 
        std::unordered_map<int, bool> blocked;
        std::unordered_map<int, std::unordered_set<int>> Blocker;
        std::vector<int> pathStack;
        std::function<bool(int, int, std::vector<std::vector<int>>&)> FindCycle;
        FindCycle=[&](int CurrNode, int Start, std::vector<std::vector<int>>& cycles){
            bool found=false;
            pathStack.push_back(CurrNode);
            blocked[CurrNode]=true;
            for(int ii:adjList[CurrNode]){
                if(ii<Start)continue; // Ignore the lower node.
                if(ii==Start){// Find it!!
                    std::vector<int> cycle=pathStack;
                    cycle.push_back(Start);
                    cycles.push_back(cycle);
                    found=true;} 
                else if(!blocked[ii]){
                    if (FindCycle(ii, Start, cycles)) {
                        found=true;}}
                else ;}
            if(found){// Reset block
                std::stack<int> unblockStack;
                unblockStack.push(CurrNode);
                while (!unblockStack.empty()) {
                    int uu=unblockStack.top();
                    unblockStack.pop();
                    blocked[uu]=false;
                    for(int vv : Blocker[uu]){
                        if(blocked[vv])unblockStack.push(vv);}
                    Blocker[uu].clear();}}
            else {// Update block
                for(int uu : adjList[CurrNode]){
                    if(uu<Start)continue;
                    if(Blocker[uu].find(CurrNode)==Blocker[uu].end()){
                        Blocker[uu].insert(CurrNode);}}}
            pathStack.pop_back();
            return found;};
        std::vector<int>sortedNodes=aSCC;// From small to large
        sort(sortedNodes.begin(),sortedNodes.end());
        for(int Start:sortedNodes){
            for(int uu:aSCC){// Reset 
                blocked[uu]=false;
                Blocker[uu].clear();}
            pathStack.clear();
            FindCycle(Start, Start, FBLs);// Find a cycle
            for (auto& [uu, neighbors] : adjList) {// Remove current points
                neighbors.erase(remove(neighbors.begin(), neighbors.end(), Start), neighbors.end());}}}
    return *this;
}


// Code is over.
// Mod 20241219: modify indeterminate argument funciton, "NetGraphFrame::ConfigurationBuildNet"
// Add 20250320: add strong-connected componets.

// in ConfigurationBuildNet()...
// va_list args;
// case 'N':// a null-model
        //     NetPara.append(" Null Model").push_back('\n');
        //     break;
        // case 'K':case 'L':case 'R':case 'B':
        // // Kauffman, Lattice, Regular-Random, Barabasi-Albert
        //     va_start(args, 1);
        //     tmp_i_arg[0]=va_arg(args, int);// "K" parameter or "Core" of BA.
        //     va_end(args);
        //     sprintf(tmpBuffer, "%d", tmp_i_arg[0]);
        //     NetPara.append(", iPara=").append(tmpBuffer).push_back('\n');
        //     break;
        // case 'E':// Erdos-Renyi
        //     va_start(args, 1);
        //     tmp_f_arg[0]=va_arg(args, double);// "AvDeg" of ER.
        //     va_end(args);
        //     sprintf(tmpBuffer, "%.4f", tmp_f_arg[0]);
        //     NetPara.append(", fPara=").append(tmpBuffer).push_back('\n');
        //     break;
        // case 'S':// Scale-free
        //     va_start(args, 3);
        //     tmp_i_arg[0]=va_arg(args, int);// MaxDeg
        //     tmp_i_arg[1]=va_arg(args, int);// MinDeg
        //     tmp_f_arg[0]=va_arg(args, double);// Gamma, scaling degree.
        //     va_end(args);
        //     sprintf(tmpBuffer, "%d", tmp_i_arg[0]);
        //     NetPara.append(", iPara1=").append(tmpBuffer);
        //     sprintf(tmpBuffer, "%d", tmp_i_arg[1]);
